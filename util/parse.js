...
export const parseQuery = (queryPath, query, context) => {
	...
	if (path === '') {
		// If path not set as param, try parsing from url
		[host, path, queryString] = parseURLPath(queryPath);
	}

	// If path still empty, get from header
	if (path === '') {
	...
	if (queryString === '') {
		// If query string empty, get from header
		queryString = context?.headers?.get('x-query-string') ?? '';
	}
