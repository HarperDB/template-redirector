...
describe('Check a regex match', async () => {
	...
	it('Should match the simple regex', async () => {
		await checkRedirect('/dir1/something', '/dir2/');
	});

	it('Should match and preserve qs', async () => {
		await checkRedirect('/dir1/something?foo=bar', '/dir2/?foo=bar');
	});

	it('Should match correct regex when similar exists', async () => {
		await checkRedirect('/dir11/another-thing', '/dir99/');
	});

	it('Should match and remove qs', async () => {
		await checkRedirect('/dir11/another-thing?arg1=val', '/dir99/');
	});

	it('Should match more complex regex', async () => {
		await checkRedirect('/dir66/anything/file5', '/magic/shopping/deals');
	});

	it('Should match and filter qs', async () => {
		await checkRedirect(
			'/dir66/anything/file5?top=1&foo=bar&fab=val',
			'/magic/shopping/deals?foo=bar'
		);
	});

	it('Should match longer regex', async () => {
		await checkRedirect('/dir77/apples/red/fruit-basket/green', '/violets/blue');
	});

	it('Should match and filter qs', async () => {
		await checkRedirect(
			'/dir77/apples/red/fruit-basket/green?flower=val&arg3=val2&foo=bar',
			'/violets/blue?arg3=val2&foo=bar'
		);
	});
});
